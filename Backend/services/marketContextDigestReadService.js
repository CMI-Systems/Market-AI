...
function validateDigestOrFail(dto) {
  ...
  if (validation.valid) {
 return { ok: true, data: dto, generatedAt: new Date().toISOString() };
  }
...
function buildMarketContextDigest(symbol = "SPY") {
  const now = new Date().toISOString();

  return {
 digestId: `market-context-${symbol.toLowerCase()}-${Date.now()}`,
 digestType: "not_ready",
 symbol,
 marketScope: symbol,
 summary: `Staging market context digest for ${symbol}. Approved normalized source is pending integration.`,
 sourceCount: 0,
 confidence: 0,
 freshnessState: "unavailable",
...
keyDrivers: [],
risks: ["approved_source_missing"],
generatedAt: now
  };
}

function listMarketContextDigests() {
  return validateDigestOrFail(buildMarketContextDigest("SPY"));
}

function getLatestMarketContextDigest(filters = {}) {
  const symbol = filters.symbol ? sanitizeText(filters.symbol).trim().toUpperCase() : "SPY";
...
return validateDigestOrFail(buildMarketContextDigest(symbol));
}
